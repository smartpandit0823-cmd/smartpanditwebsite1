...
export default async function OffersPage() {
  await auth();
  await connectDB();
  const offers = await Offer.find({}).sort({ createdAt: -1 }).lean() as any[];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Title</TableHead><TableHead>Type</TableHead><TableHead>Discount</TableHead><TableHead>Active</TableHead></TableRow></TableHeader><TableBody>{offers.map((o: any) => (<TableRow key={o._id.toString()}><TableCell><Link href={`/admin/offers/${o._id.toString()}`} className="font-medium">{o.title}</Link></TableCell><TableCell>{o.type}</TableCell><TableCell>{o.discount}{o.discountType === "percent" ? "%" : "₹"}</TableCell><TableCell><Badge variant={o.active ? "success" : "secondary"}>{o.active ? "Active" : "Inactive"}</Badge></TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
    </div>
  );
}
