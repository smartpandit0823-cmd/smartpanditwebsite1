...
export default async function SlidersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Title</TableHead><TableHead>Order</TableHead><TableHead>Active</TableHead></TableRow></TableHeader><TableBody>{sliders.map((s: { _id: string; title: string; order: number; active: boolean }) => (<TableRow key={s._id}><TableCell><Link href={`/admin/sliders/${s._id}`} className="font-medium">{s.title}</Link></TableCell><TableCell>{s.order}</TableCell><TableCell><Badge variant={s.active ? "success" : "secondary"}>{s.active ? "Active" : "Inactive"}</Badge></TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
    </div>
  );
}
