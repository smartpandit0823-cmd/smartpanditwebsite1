...
export default async function TransactionsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Order ID</TableHead><TableHead>Customer</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead></TableRow></TableHeader><TableBody>{result.data.map((t: { _id: string; razorpayOrderId: string; customerName: string; amount: number; status: string; createdAt: string }) => (<TableRow key={t._id}><TableCell className="font-mono text-sm">{t.razorpayOrderId?.slice(0, 20)}...</TableCell><TableCell>{t.customerName}</TableCell><TableCell>{formatCurrency(t.amount)}</TableCell><TableCell><Badge variant={t.status === "captured" ? "success" : t.status === "refunded" ? "destructive" : "secondary"}>{t.status}</Badge></TableCell><TableCell>{formatDateTime(t.createdAt)}</TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
    </div>
  );
}
